...
public:
    ...
    ExampleApplication() : 
        window(new MYR::Window(WIDTH,HEIGHT)),
        core(new MYR::Core(enableValidationLayers)),
        device(new MYR::Device()),
        swapChain(new MYR::SwapChain(device)),
        pipeline(new MYR::Pipeline(device)),
        command(new MYR::Command(device,pipeline,swapChain,MAX_FRAMES_IN_FLIGHT)),
        imageManager(new MYR::ImageManager(device,command)),
        bufferManager(new MYR::BufferManager(device, command)),
        buffers(new MYR::Buffers(device,pipeline,command,MAX_FRAMES_IN_FLIGHT)),
        camera(new Camera())
    {
    }
